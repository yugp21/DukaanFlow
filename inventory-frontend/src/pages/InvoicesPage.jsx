    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', background: 'rgba(15,23,42,0.6)', backdropFilter: 'blur(8px)' }}>
      <div style={{ background: 'white', borderRadius: '20px', width: '100%', maxWidth: '500px', boxShadow: '0 25px 60px rgba(0,0,0,0.2)', overflow: 'hidden', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ padding: '24px' }}>
          ...
          <h4 style={{ fontSize: '13px', fontWeight: 600, color: '#374151', margin: '0 0 10px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Items</h4>
          ...
          <div style={{ background: '#1e3a5f', borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
              <span style={{ color: 'rgba(255,255,255,0.7)', fontSize: '13px' }}>Subtotal</span>
              <span style={{ color: 'white', fontSize: '13px' }}>₹{Number(invoice.subtotal).toLocaleString('en-IN')}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
              <span style={{ color: 'rgba(255,255,255,0.7)', fontSize: '13px' }}>Discount ({invoice.discount}%)</span>
              <span style={{ color: 'white', fontSize: '13px' }}>— ₹{(Number(invoice.subtotal) * Number(invoice.discount) / 100).toFixed(2)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
              <span style={{ color: 'rgba(255,255,255,0.7)', fontSize: '13px' }}>Tax ({invoice.taxPercent}%)</span>
              <span style={{ color: 'white', fontSize: '13px' }}>+ ₹{(Number(invoice.subtotal) * (1 - Number(invoice.discount)/100) * Number(invoice.taxPercent) / 100).toFixed(2)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: '10px', borderTop: '1px solid rgba(255,255,255,0.15)' }}>
    ...
    <div style={{ fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          ...
          <p style={{ fontSize: '13px', color: '#64748b', margin: 0 }}>{invoices.length} total invoices</p>
        </div>
        <div style={{ background: 'linear-gradient(135deg, #0f2744, #1e3a5f)', borderRadius: '12px', padding: '10px 16px', textAlign: 'right' }}>
          <p style={{ color: '#93c5fd', fontSize: '11px', margin: '0 0 2px' }}>Total Revenue</p>
            ...
            {filtered.map(inv => (
              <div key={inv.id} style={{ background: '#f8fafc', borderRadius: '12px', padding: '14px', border: '1px solid #f1f5f9' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '10px' }}>
                  <div>
                    <p style={{ fontSize: '13px', fontWeight: 700, color: '#2563eb', margin: '0 0 2px', fontFamily: 'monospace' }}>{inv.invoiceNumber}</p>
                    <p style={{ fontSize: '14px', fontWeight: 600, color: '#0f172a', margin: '0 0 2px' }}>{inv.customerName}</p>
                    <p style={{ fontSize: '12px', color: '#64748b', margin: 0 }}>{new Date(inv.createdAt).toLocaleDateString('en-IN')} · {inv.paymentMethod}</p>
                  </div>
