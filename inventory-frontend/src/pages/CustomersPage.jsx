  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(15,23,42,0.6)', backdropFilter: 'blur(8px)', padding: '16px' }}>
      <div style={{ background: 'white', borderRadius: '20px', padding: '28px', maxWidth: '360px', width: '100%', textAlign: 'center', boxShadow: '0 25px 60px rgba(0,0,0,0.2)', animation: 'modalIn 0.2s ease' }}>
        ...
        <h3 style={{ fontSize: '17px', fontWeight: 700, color: '#0f172a', margin: '0 0 8px' }}>Delete Customer?</h3>
...
function Modal({ title, onClose, children }) {
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', background: 'rgba(15,23,42,0.6)', backdropFilter: 'blur(8px)' }}>
      <div style={{ background: 'white', borderRadius: '20px', width: '100%', maxWidth: '440px', boxShadow: '0 25px 60px rgba(0,0,0,0.2)', overflow: 'hidden', animation: 'modalIn 0.2s ease' }}>
...
export default function CustomersPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await client.put(`/api/customers/${editing.id}`, form)
  ...
  const confirmDeleteAction = async () => {
    try {
      await client.delete(`/api/customers/${confirmDelete}`)
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h1 style={{ fontSize: isMobile ? '20px' : '26px', fontWeight: 700, color: '#0f172a', margin: '0 0 4px' }}>Customers</h1>
          <p style={{ fontSize: '13px', color: '#64748b', margin: 0 }}>{customers.length} total customers</p>
        </div>
              ...
              <div key={c.id} style={{ background: '#f8fafc', borderRadius: '12px', padding: '14px', border: '1px solid #f1f5f9' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'linear-gradient(135deg, #7c3aed, #a78bfa)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <span style={{ color: 'white', fontSize: '13px', fontWeight: 700 }}>{c.name?.charAt(0).toUpperCase()}</span>
                    </div>
                    <div>
                      <p style={{ fontSize: '14px', fontWeight: 700, color: '#0f172a', margin: 0 }}>{c.name}</p>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                  <div style={{ background: 'white', borderRadius: '8px', padding: '8px 10px' }}>
                    <p style={{ fontSize: '10px', color: '#94a3b8', margin: '0 0 2px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Email</p>
                    <p style={{ fontSize: '12px', color: '#374151', margin: 0, wordBreak: 'break-all' }}>{c.email || '—'}</p>
                  </div>
                  <div style={{ background: 'white', borderRadius: '8px', padding: '8px 10px' }}>
                    <p style={{ fontSize: '10px', color: '#94a3b8', margin: '0 0 2px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Total Purchases</p>
                    <p style={{ fontSize: '13px', fontWeight: 700, color: '#0f172a', margin: 0 }}>₹{Number(c.totalPurchases || 0).toLocaleString('en-IN')}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#f8fafc' }}>
                {['Customer', 'Phone', 'Email', 'Address', 'Total Purchases', ''].map(h => (
            ...
            <tbody>
              {filtered.map((c, i) => (
                ...
                >
                  ...
                  <td style={{ padding: '14px 20px', fontSize: '13px', color: '#64748b' }}>{c.phone || '—'}</td>
                  <td style={{ padding: '14px 20px', fontSize: '13px', color: '#64748b' }}>{c.email || '—'}</td>
                  <td style={{ padding: '14px 20px', fontSize: '13px', color: '#64748b' }}>{c.address || '—'}</td>
                  <td style={{ padding: '14px 20px', fontSize: '14px', fontWeight: 700, color: '#0f172a' }}>₹{Number(c.totalPurchases || 0).toLocaleString('en-IN')}</td>
                  <td style={{ padding: '14px 20px' }}><ActionButtons c={c} /></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {showModal && (
        <Modal title={editing ? '✏️ Edit Customer' : '➕ Add Customer'} onClose={() => setShowModal(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            {[
              { label: 'Full Name', key: 'name', type: 'text', placeholder: 'e.g. Raj Shah' },
              { label: 'Phone', key: 'phone', type: 'tel', placeholder: 'e.g. 9876543210' },
              { label: 'Email', key: 'email', type: 'email', placeholder: 'e.g. raj@gmail.com' },
              { label: 'Address', key: 'address', type: 'text', placeholder: 'e.g. Anand, Gujarat' },
